...
many_boost.py
-------------
Per-scoreset ensemble where each scoreset is modeled by its own XGBoost regressor.
Mirrors the per-scoreset approach of the neural ensemble but swaps the per-scoreset
MLPs for gradient boosted trees.
"""
...
class ManyBoostModel(Model):
    """
    Trains one XGBoost regressor per scoreset.
    """
    ...
    def __init__(self, params: Dict[str, Any]) -> None:
        ...
        self.early_stopping_rounds: int = int(params.get("early_stopping_rounds", 20))
        ...
        # simple categorical encoders
        self.encoders: Dict[str, Dict[Any, int]] = {}
        self.feature_names: List[str] = []

        # store val splits for reporting
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        """
        Encode categoricals and store feature vectors in a single column to avoid creating
        millions of scalar tensors (which would exhaust memory).
        """
        df = data.copy()
    ...
    def forward(self, x: List[Tensor]) -> Tensor:
        """
        x[0]: features tensor, x[1]: scoreset_id tensor
        """
        if len(x) != 2:
    ...
    def train_loop(
        ...
        """
        Train one XGBRegressor per scoreset. Also stores per-scoreset val splits.
        """
        if XGBRegressor is None:
            raise ImportError("xgboost is required for ManyBoostModel but is not installed.")

        # Reconstruct numpy arrays from dataset (each row -> [feature_vector, scoreset_id])
        ...
        unique_scoresets = np.unique(scoreset_ids)
        for sid in tqdm(unique_scoresets, desc="Training XGB per scoreset"):
            ...
            X_train, y_train = X_sid[train_idx], y_sid[train_idx]
            X_val, y_val = X_sid[val_idx], y_sid[val_idx]
            self.val_sets[int(sid)] = (
                torch.tensor(X_val, dtype=torch.float32, device=dataset.device),
                torch.tensor(y_val, dtype=torch.float32, device=dataset.device),
            )
    ...
    @staticmethod
    def load(file_path: str) -> "ManyBoostModel":
        import pickle

        with open(file_path, "rb") as f:
            payload = pickle.load(f)
        model = ManyBoostModel(params={})
        ...
        model.early_stopping_rounds = payload.get("early_stopping_rounds", model.early_stopping_rounds)
    ...
    def prepare_inference(self, data: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, List[Any]]:
        """
        Build inference features using stored encoders. Returns (features, scoreset_ids, accessions).
        """
        df = data.copy()
