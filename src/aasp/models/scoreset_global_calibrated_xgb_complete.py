...
class ScoresetGlobalCalibratedXGBCompleteModel(Model):
    """
    Same as ScoresetGlobalCalibratedXGBModel but intended for full-dataset training (no val split).
    """
    ...
    def train_loop(
        ...
        progress_bar = tqdm(total=self.xgb_params.get("n_estimators", 0), desc="Training global XGB (complete)")
        ...
        self.calibrators = {}
        for sid in tqdm(np.unique(sid_arr), desc="Fitting calibrators (complete)"):
            mask = sid_arr == sid
        ...
        try:
            ...
            self.save("models/saved_models/global_calibrated_xgb_complete")
        except Exception:
            pass
    ...
    @staticmethod
    def load(file_path: str) -> "ScoresetGlobalCalibratedXGBCompleteModel":
        import pickle

        with open(file_path, "rb") as f:
            payload = pickle.load(f)

        model = ScoresetGlobalCalibratedXGBCompleteModel(params={"xgb_params": payload.get("xgb_params", {})})
        booster = xgb.Booster()
